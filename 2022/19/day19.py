import re, math; data = open('input.txt').readlines()
blueprints=[[(a,0,0,0),(b,0,0,0),(c,d,0,0),(e,0,f,0)] for a,b,c,d,e,f in [map(int,re.findall(r'\d+',line)[1:]) for line in data]]
...
print(sum(i*dfs(bp, 23) for i,bp in enumerate(blueprints,1))); print(math.prod(dfs(bp, 31) for bp in blueprints[0:3]))
